    ...
    member x.CountryTradeReport () =
        ...
        |> Seq.map (fun (abbr, col) ->
            { country = countryMap.[abbr].DisplayName
              ...
              power = col |> Seq.sumBy (fun x -> x.Value)
...
//        let buildings = x.CountryBuildings ()
        ...
        let playerCountries = 
            save.Countries
            |> Seq.where (fun x -> x.Human.GetValueOrDefault() ||
                                   x.WasPlayer.GetValueOrDefault())
        let names =
            match isNull save.PlayersCountries with
            | true -> [((Seq.head playerCountries).DisplayName, "human")] 
                      |> List.toSeq
            | false -> 
        ...
        |> Seq.map (fun (player, country, builds) ->
            ...
            { name = country.DisplayName;
              ...
              baseTax = tax;
              admTech = country.Technology.AdmTech;
              dipTech = country.Technology.DipTech;
              milTech = country.Technology.MilTech;
